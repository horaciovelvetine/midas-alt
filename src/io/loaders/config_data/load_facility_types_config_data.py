...
def load_facility_types_config_data(excel_file: ExcelFile, result: ConfigDataLoadResult) -> dict[int, FacilityType]:
    """Load ``FacilityType`` instances from the ``Facilities`` config data excel sheet."""
    from src.config.midas_settings import MidasSettings

    if "Facilities" not in excel_file.sheet_names:
        result.add_warning(f"Unable to find the 'Facilities' sheet in the {MidasSettings.DEFAULT_CONFIG_DATA_FILENAME} excel file.")
        return {}
    ...
    logger.info(
        "Loaded %d Facility Type(s) from the 'Facilities' sheet in the %s excel file.",
        len(facility_types),
