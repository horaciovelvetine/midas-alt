...
def build_mission_alert_panel(session: SimulationSession) -> Panel | None:
    ...
    hint.append(" for rules, counts, and per-alert drill-down (metrics + why + sample WOs).", style="dim")
    return Panel(
...
def _mission_alert_work_order_sample_table(wos: list[WorkOrder], *, max_rows: int) -> Panel:
    ...
    if len(wos) > max_rows:
        ...
        return Panel(Group(tbl, note), title=f"Open mission-impacting WOs (first {max_rows})", border_style="magenta")
...
def collect_mission_alert_items(session: SimulationSession) -> list[MissionAlertItem]:
    ...
    if (
        inst_state.mission_impacting_open_work_orders >= INSTALL_MISSION_WO_ALERT_THRESHOLD
